...
export const getExpenses = async (req, res) => {
    ...
    const query = { userId }; //query will be specific distinct userId
    const now = new Date();
    let start; //declare a start date
    if (filter === "week") {
        start = new Date();
...
export const updateExpense = async (req, res) => {
    ...
    const expense = await expenseModel.findOneAndUpdate(filter, req.body, { new: true });
    if (!expense) {
